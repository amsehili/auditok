...
class TestADSFactoryFileAudioSource:
    ...
    def test_ADS_type(self):
        ...
        err_msg = (
            "wrong type for ads object, expected: 'AudioDataSource', found: {0}"
        )
        assert isinstance(ads, AudioDataSource), err_msg.format(type(ads))
    ...
    def test_hop_duration(self):
        ...
        size = ads.hop_size
        assert size == 160, "Wrong hop_size, expected: 160, found: {0}".format(
            size
        )
    ...
    def test_channels(self):
        ...
        assert (
            channels == 1
        ), "Wrong number of channels, expected: 1, found: {0}".format(channels)
    ...
    def test_read(self):
        ...
        assert ads_data == audio_source_data, "Unexpected data read from ads"
    ...
    def test_Limiter_Deco_read_limit(self):
        ...
        expected_read_bytes = total_samples_with_overlap * ads.sw * ads.channels
    ...
    def test_Recorder_Deco_is_rewindable(self):
        ...
        assert ads.rewindable, "RecorderADS.is_rewindable should return True"
    ...
    def test_Overlap_Deco_read(self):
        ...
        # Compare all blocks read from OverlapADS to those read from an audio source with a manual position setting
...
class TestADSFactoryBufferAudioSource:
    def setup_method(self):
...
class TestADSFactoryAlias:
    ...
    def test_block_duration_alias(self):
        ...
        size = ads.block_size
        err_msg = "Wrong block_size set with a block_dur alias 'bd', expected: 8, found: {0}"
        assert size == 12, err_msg.format(size)
    ...
    def test_record_alias(self):
        ...
        assert ads.rewindable, "AudioDataSource.rewindable expected to be True"
    ...
    def test_Limiter_Recorder_Overlap_Deco_rewind_and_read_alias(self):
        ...
        # Compare all blocks read from AudioDataSource to those read from an audio source with manual position definition
        for j in range(i):
            tmp = audio_source.read(block_size)
            block = ads.read()
            assert (
